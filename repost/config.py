...
# 1500 is an optional editorial target, never a publishing hard limit.
MAX_POST_CHARS = max(1, min(int(_get("MAX_POST_CHARS") or 1500), 1500))
THREAD_PLATFORM_CHARS = 500
THREAD_ITEM_CHARS = max(
    100,
...
LIMITS = {
    "linkedin": LINKEDIN_POST_CHARS,
    "twitter": X_PREMIUM_POST_CHARS if X_PREMIUM else 280,
    "threads": THREAD_PLATFORM_CHARS,
}
